...
func (c *FederatedHPAController) getBindingByLabel(resourceLabel map[string]string, resourceRef autoscalingv2.CrossVersionObjectReference) (*workv1alpha2.ResourceBinding, error) {
	...
	if _, ok := resourceLabel[policyv1alpha1.PropagationPolicyUIDLabel]; ok {
		policyUID := resourceLabel[policyv1alpha1.PropagationPolicyUIDLabel]
		selector = labels.SelectorFromSet(labels.Set{
			policyv1alpha1.PropagationPolicyUIDLabel: policyUID,
		})
	} else if _, ok = resourceLabel[policyv1alpha1.ClusterPropagationPolicyUIDLabel]; ok {
		policyUID := resourceLabel[policyv1alpha1.ClusterPropagationPolicyUIDLabel]
		selector = labels.SelectorFromSet(labels.Set{
