...
func (c *Controller) tryDeleteWorkload(clusterName string, work *workv1alpha1.Work) error {
	for _, manifest := range work.Spec.Workload.Manifests {
		...
		if util.GetLabelValue(clusterObj.GetLabels(), workv1alpha1.WorkRefrenceKey) != string(work.GetUID()) {
...
func (c *Controller) syncToClusters(clusterName string, work *workv1alpha1.Work) error {
	...
	for _, manifest := range work.Spec.Workload.Manifests {
		...
		util.MergeLabel(workload, workv1alpha1.WorkRefrenceKey, string(work.UID))
		err := workload.UnmarshalJSON(manifest.Raw)
		if err != nil {
			klog.Errorf("Failed to unmarshal workload, error is: %v", err)
