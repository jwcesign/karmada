...
type bindingCoreParameters struct {
...
func ensureWork(
	...
	bindingCorePara := extractBingCoreParameters(binding, scope)
	bindingCorePara.targetClusters = mergeTargetClusters(bindingCorePara.targetClusters, bindingCorePara.requiredByBindingSnapshot)

	var jobCompletions []workv1alpha2.TargetCluster
	var err error
	if workload.GetKind() == util.JobKind {
		jobCompletions, err = divideReplicasByJobCompletions(workload, bindingCorePara.targetClusters)
		if err != nil {
			return err
		}
	}

	for i := range bindingCorePara.targetClusters {
		clonedWorkload := workload.DeepCopy()
		targetCluster := bindingCorePara.targetClusters[i]
		...
		if needReviseReplicas(bindingCorePara.replicas, bindingCorePara.placement) {
		...
		annotations = mergeConflictResolution(clonedWorkload, bindingCorePara.conflictResolutionInBinding, annotations)
...
func extractBingCoreParameters(binding metav1.Object, scope apiextensionsv1.ResourceScope) *bindingCoreParameters {
	ret := &bindingCoreParameters{}
	switch scope {
