...
func (c *ResourceBindingController) Reconcile(ctx context.Context, req controllerruntime.Request) (controllerruntime.Result, error) {
	...
	if !binding.DeletionTimestamp.IsZero() {
		...
		if err := helper.DeleteWorkByResourceBinding(c.Client, &binding.ObjectMeta); err != nil {
	...
	if util.GetLabelValue(binding.GetLabels(), workv1alpha2.ResourceBindingIDLabel) == "" &&
		util.GetLabelValue(binding.GetLabels(), workv1alpha2.ClusterResourceBindingIDLabel) == "" {
