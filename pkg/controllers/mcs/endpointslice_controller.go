...
func (c *EndpointSliceController) Reconcile(ctx context.Context, req controllerruntime.Request) (controllerruntime.Result, error) {
	...
	if !work.DeletionTimestamp.IsZero() {
		err := helper.DeleteEndpointSlice(c.Client, labels.Set{
			workv1alpha2.WorkUIDLabel: req.Namespace,
		})
		if err != nil {
...
func (c *EndpointSliceController) addFinalizer(work *workv1alpha1.Work) error {
	if controllerutil.ContainsFinalizer(work, util.MCSControllerFinalizer) {
		return nil
	}

	controllerutil.AddFinalizer(work, util.MCSControllerFinalizer)
...
func (c *EndpointSliceController) removeFinalizer(work *workv1alpha1.Work) error {
	...
	controllerutil.RemoveFinalizer(work, util.MCSControllerFinalizer)
	err := c.Client.Update(context.TODO(), work)
	if err != nil {
		return err
	}
