...
func (d *DependenciesDistributor) addFinalizer(resourceBinding *workv1alpha2.ResourceBinding) error {
	if !controllerutil.AddFinalizer(resourceBinding, util.DependenciesDistributorFinalizer) {
		return nil
	}

	patch := client.MergeFrom(resourceBinding)
	if err := d.Client.Patch(context.TODO(), resourceBinding, patch); err != nil {
		return err
	}
...
func (d *DependenciesDistributor) removeFinalizer(resourceBinding *workv1alpha2.ResourceBinding) error {
	if !controllerutil.RemoveFinalizer(resourceBinding, util.DependenciesDistributorFinalizer) {
		return nil
	}
